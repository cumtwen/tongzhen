...
public class OrderController {
	...
	public ModelAndView search(ModelAndView mv,HttpServletRequest request,HttpSession session){
		int user_id=(Integer)session.getAttribute("numb");
		int id =Integer.parseInt(request.getParameter("id"));
		Order order=orderService.selectOrer(user_id,id);
		if(order!=null){
			session.setAttribute("order", order);
			mv.setViewName("order");;
		}else{
			mv.addObject("message","无相关订单！");
			mv.setViewName("order");
		}
