...
<!-- 
	<div class="container">
		<div class="check-out">
    	    <h4 class="title">您还没有收藏任何东西哟</h4>
    	    <p class="cart"><a href="index"> 点击我</a>再去逛逛吧</p>
    	</div>
  ...
  <div>
     ...
     <c:if test="${message!=null }"><c:out value="${message}" /></c:if>
         <h1><a class="btn btn-primary" href="index" role="button">继续购物</a>
            <a class="btn btn-danger" href="removecollect" role="button">清空收藏夹</a></h1>
         <br />
         <table class="table table-hover">
           ...
           <c:forEach items="${requestScope.collect }" var="collect">
			<tr>
				<td>${collect.good_id}${clothes.id}</td>
				<td>${collect.good_price}</td>
			<td>
			<a class="btn btn-info" href="order?good_id=${collect.id}&good_price=${collect.good_price}&count=1" role="button">立即购买</a>
		    </td>
		    <td>
			<a class="btn btn-danger" href="delcollect?good_id=${collect.good_id}" role="button">删除</a>
		    </td>
		  </tr>
		</c:forEach>
        </table>
